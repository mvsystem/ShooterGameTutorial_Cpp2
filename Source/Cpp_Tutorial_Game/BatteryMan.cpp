...
ABatteryMan::ABatteryMan()
{
	...
	bool mv = PrimaryActorTick.bCanEverTick = true;
	FString Name = this->GetName();
	FVector Location = this->GetActorLocation();
	
	UE_LOG(LogTemp,Warning,TEXT("%s:%s:%s"),*this->GetName(),*Location.ToString(),(mv ? TEXT("TRUE") : TEXT("FALSE")))
	...
	GetCharacterMovement()->RotationRate = FRotator(2.0f, 540.0f, 0.0f);
                                 ...
                                 int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	{
		//UE_LOG(LogTemp, Warning, TEXT("COlided With"));
...
void ABatteryMan::NotifyActorBeginOverlap(AActor* OtherActor)
{
	Super::NotifyActorBeginOverlap(OtherActor);

	UE_LOG(LogTemp,Warning,TEXT("%s:%s"),*OtherActor->GetName(),*this->GetName())
}
